...
- **e2e 矩阵覆盖洞**：`test/e2e/all_phases_test.go` 覆盖 p1–p10 + p13 + 叶子矩阵（TransferDefaults / RemoteFS / ProxyTunnelReconnect / **ProxyDial** / **D1** / **D2** / **D3** / **D4**）；**p11 及 post-1.0 的 file-transfer / ps-retention / P12 仍未进矩阵**。新增量进矩阵时一并回填。
...
- **D4 DONE（外审 round-2 re-review PASS，已 push main，`4ca8891`）**；下一步 **D5**（审计可重导 + JS 流副本重配）：§6.3 leader post-commit 单写 `ev.*`/`audit.*`/`sys.events` + 审计=committed entry+`raft_index` 纯函数(任新 leader 从复制 entry 重导、不依赖 leader-local 态) + dedup 窗口 > (选举+扫尾) 最大时延 + 选后幂等 sweep；§6.4/§9 `history-<sid>`/`OBJ_xfer-<sid>` `Replicas=replicasFor(nVoters)` + already-in-use 分支 actual<target 即 `UpdateStream` + 全流达 target 前禁 retire + `replication_degraded`。**D4 已交付自足重导原语** `proc.ReplayReconcileAudit`（D5 接 post-commit 发布 + `raft_index:kind:seq` 去重窗口）。注意 proto v2 不发现网 v1 车队。
