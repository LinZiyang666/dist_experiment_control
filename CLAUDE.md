...
- **D8 DONE（外审 re-review PASS，已 push main，`f57ae52`）**；下一步 **D9**（一次性迁移 + nats.conf 接管 + 发布硬化 = HA GA，§11/§19-D9）：§11 `cluster init --from-existing`（前向 0008–0013 + `BootstrapCluster({self})` + 迁移后 DB 作 index-0 快照 + 删 `broker.New` 启动期写）；**tether 接管 `nats-server.conf`**（`.bak` + before/after 归属表 + preflight 遇不认识指令拒接管 + 与 install.sh 产物对账）；**同 PR 改写 `usage.md` §2.3/§3.4**；§15 provisioning 全套 secret 分发 runbook + FDE preflight；§8.3 account.nk/CA 轮换 runbook。**这是真 cutover**：把 D2–D8 全部 build-and-prove 的接缝接到生产——`serve.go`/`broker.New` 构造 `cluster.Node`、注册 adminsock cluster backend + alert reconcile/audit publisher loop + transfer 全部 sink、daemon 端取 disk lock（D7 的 (C) inert-until-D9 层）。**测试**：3 节点实跑全演练 + mass-reconnect e2e 门 + FDE preflight + `--from-existing` 半跑/回滚幂等 + **新 phase 全进 e2e 矩阵（回填 p11/file-transfer/ps-retention/P12 等历史洞）**。出口：现网单点一次性迁成 N=1 → `cluster add` 长到 N≥3 → §17 全保证矩阵达成 → GitHub release。注意 proto v2 不发现网 v1 车队（patch 走 v1 线，[[project-release-lines-v1-v2]]）。D9 需用户新 /goal 启动。
