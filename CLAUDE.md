...
- **D6 DONE（外审 round-2 re-review PASS，已 push main，`d403f38`）**；下一步 **D7**（集群生命周期 + 逃生）：§8.1 命令面（admin 严格本地、非 leader fail-fast 指名 leader）+ **membership 两阶段**（阶段 1 `ClusterNodeUpsert{join PoP}` follower Apply 复算 → 阶段 2 `raft.AddVoter`、半成功 phase 状态机、Remove/retire 顺序、幂等）；§8.2 catch-up 闸；§8.3 create/online/**drain**（迁 expose 走 D6 rehome + quorum 投影守卫 + secret 全套分发）/retire；§8.4 **force-single offline 子命令 + self-fence(`T_fence`) + runbook**（`systemctl mask`/空缺拒/调和两存储/peer 可达 hard-refuse）；§17 `status/doctor` 双视图(ctl/NATS vs broker offline)+`--json`+退出码。**依赖**：D7 drain 迁 expose 依赖 D6 rehome（先父后子，已就位）；`cluster_nodes` 生产写者 + `cluster rotate-tunnel-cert` CLI + 动态 AddVoter/join-PoP 是 D7 首做（D6 build-and-prove 由 harness 播种）。注意 proto v2 不发现网 v1 车队。D7 需用户新 /goal 启动。
