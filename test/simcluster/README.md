...
| `71-expose-rehome-failover` | **RED-EXPOSES (G-A/S3, N=3)** the drain-migrate gap as release-blocking (R6-M2) + **gotcha #29** — a cluster expose's home is NOT deliverable to a non-tunnel broker (net effect = tunnel-coupling, but the MECHANISM is un-homed fallback, R5-M3): `homeForExpose` (home.go:96-113) returns nil for a `--on-broker <non-tunnel voter>` → the agent's AddProxy falls back to its FIXED tunnel broker (tunnel_adapter.go:76-77) → REGISTER denied `token_unknown_or_revoked` (home_broker != self) → `agent_rejected:frpc_failed` (this is NOT "the agent hardcodes its tunnel by design" — the round-4 wording the source refuted; the agent WOULD dial a delivered named home, cf. #33's proxy exit). AND a crashed home STRANDS a regular expose cluster-wide (rehome_events.go:52-53). **FIXTURE gate (HARD, R6-M2)**: agt→brk3 tunnel is INTERMITTENT — a HARD assertion RED-exposes it when it doesn't establish in 200s (never a silent GREEN NOT-COVERED). **Combined crash (Arm C rebuild-ON + Arm D rebuild-OFF, ONE injection)**: agt tunneled to brk3, `wstrand` (rebuild-ON) + `wnr` (rebuild-OFF) both homed brk3 → `node_kill brk3` → BOTH dead on EVERY live voter (curl exit 7, F5) + `cluster status` reachability proves the leader SAW the crash + epoch UNCHANGED + wnr explain honest (rebuild:false, not moved) → `node_start brk3` → BOTH recover on the SAME port/epoch (rebuild-OFF crash behaviour IS the injection, R5-M3). **Arm A**: `on_broker_unknown` negative + no row. **HARD-GATE**: brk3 is a live non-leader. **HARD RED GATE — drain-migrate (B, R6-M2)**: `cluster drain brk3` MUST migrate rebuild-ON wstrand to a survivor voter that SERVES — RED when blocked by the three walls, EXPOSING the gap as release-blocking (NOT a self-authored owner-decision NOT-COVERED, which R6-M2 rejected): (1) homeForExpose un-homed fallback (`--on-broker <non-leader>` never serves, 180s×2); (2) agent-tunnel-to-non-leader is intermittent (the reviewer's solo1b hit this as a 71 RED); (3) even when established, `cluster drain brk3` is refused by a lingering grow `NATS_ROLLED_OUT` op (#31 family, needs manual `cluster ops abort`). **Arm E (rebuild-OFF drain refusal) is now DIRECTLY EXECUTED (R7-M2)**: with `wnr` (rebuild:false) + `wstrand` both live after return, `cluster drain brk3` is invoked and MUST refuse to auto-migrate the rebuild-OFF expose (clusterdrain.go:665 signature); if the #31 lingering op intercepts first (`NATS_ROLLED_OUT`), the signature is unreachable → RED exposes that (E no longer grouped with G/F). G/F still need a SUCCESSFUL drain as precondition → blocked by the same walls (exposed, not rescoped). **Hermetic-coverage (R8-M3 correction of the R7 scoping error)**: the drain **marker + phase-advance + rehome-target-exclusion** ARE hermetic-tested (clusteradmin_test, g1g7 A9), AND the **rebuild-OFF drain refusal** IS hermetic-tested — `test/d7/integration_test.go testD7DrainRefusesRebuildOff` asserts `errors.As ErrRebuildOffExposes` + the enumerated refused port + the home NOT silently changed (the round-7 "no `_test.go` references `ErrRebuildOffExposes`" claim was FALSE — that grep searched only `internal/`, missing `test/d7/`). Drill 71 Arm E ADDS CLI / real-stack / #31-interaction coverage on top, it is NOT the only coverage. What drill 71 UNIQUELY covers is the **rebuild-ON drain-MIGRATE end-to-end DATA PLANE** (a rebuild-ON expose actually migrating to a survivor + serving over the real tunnel) — D7's `DrainRetireFollower` no-ops `migrateExposes` (no exposes homed), so that data-plane path is NOT hermetic-tested; drill 71 Arm B is its only coverage. Raw sys.events have no operator reader (owner-decisions D2, raw-event only) |
...
| `30-rolling-upgrade` | GREEN (G-A/S5, N=3 cluster + ctl; G5 #8) — headline finding **gotcha #31**: grow's best-effort `releaseGrowLock` LEAKS `cluster_grow_active` (grow reports success / exit-0 / all-VOTER while the lock stays set), which BLOCKS the real upgrade at acquire-lock (`a membership operation is in flight`, exit 69). INVISIBLE to dry-run/refuse (neither acquires the lock — those PASS). **[GAP #31]**: real roll attempt-1 HALTs → asserted as the exposed defect; tether's OWN recovery (re-run `cluster add`) is RETRIED but walks the SAME best-effort release path and **CANNOT clear** the leaked lock → the upgrade-roll mechanism (re-exec / write-continuity / PID-same) is **NOT-COVERED** (blocked by #31 until fixed, like #29's rehome data plane). MainPID-same / write-probe-clean are deliberately **SUPPRESSED** when the roll didn't run — they'd be FALSE-GREENS (the PID stays same precisely BECAUSE no upgrade happened). Covered cleanly: privileged staging (labeled, NOT automatic) + `--expect-sha256` anchor + broker-local ctl login (upgrade is a ctl-over-NATS op, unlike admin-socket rebalance) + `--account-seed`/`--backup-taken` refuses + dry-run ordered-roll preview + a RUNNING-version oracle (self-reported via node ls, NOT the on-disk binary — staging already overwrote that). #31 is pinned here purely by the upgrade real-roll HALT [GAP #31]; it is DISTINCT from the concurrency VOTER-timeout flake (7-way parallel sweep = clustered-JS formation timing — `run-drills.sh` surfaces that RED, does NOT auto-retry it — simcluster:223-229). Stage-C: M1 write-continuity now has a non-vacuity WROTE-wp* guard (FAILs if the probe never wrote — mandate-2/pin-1) + a roll-ORDER oracle (leader's UPGRADE is last — cluster-1); 30-C N=2 negative-control drop given a stated reason (13 assertions) |
