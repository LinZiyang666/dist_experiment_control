...
| `42-rejoin-returning` | **INCOMPLETE** | S6, N=2: the returning-node recovery surface — diagnose pos/neg (alive-refuse + dead-peer pasteable) + rejoin-prepare O_EXCL WIPE-REFUSED on an intact DB + resnapshot single-voter + Tier-2/machine-confirm. Arm-A rejoin-diagnostic journal-catch + DOC-2 + E/F/I regrow are `not_covered()` |
