...
| `31-node-upgrade-fleet` | **INCOMPLETE** (G-A/S5, N=1 cluster + 2 agents): **#28-FIXED** — configured agent `upgrade.url_allow` is proven to admit the self-hosted artifact, while the negative local-allowlist discriminator remains non-vacuous. The drill covers broker negatives plus the fleet CONTROL surface: ONLINE enumeration, OFFLINE exclusion, dispatch/config-abort, `--timeout 0` transient-skip and summary. A successful fleet upgrade still needs destructive PID-preserving re-exec, version/re-register and rollback/`--wait` oracles; that deploy-tier arm is NOT-COVERED and currently has no owner. |
