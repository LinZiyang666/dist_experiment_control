...
(`… waiting for: brkN reaches VOTER`) does **not** match `FLAKE_SIG` (`run-drills.sh:57`), so it is never
auto-re-run — `-j 6` thus *maximizes* the very grow-timing REDs that then need manual single re-runs.
