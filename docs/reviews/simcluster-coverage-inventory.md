...
| `node upgrade <nid>\|--all` | — | `--url/--sha256`（S5-31✓）；**#28 已修**（R15：agent 侧 `upgrade.url_allow` 可配，31 drill 以 #28-FLIPPED 前置验证）；`--all` **enumeration/dispatch/`--timeout` COVERED**（外审 M4 可执行臂：ONLINE 枚举 + OFFLINE 排除 + dispatch-到-online + `url_not_allowed_local` config-abort + `--timeout 0` transient-skip 汇总）；fleet **SUCCESS**（真 re-exec / re-register / rollback oracle）**NOT-COVERED 且尚无 owner**——31 drill 明示归属一个 dedicated success drill，该 drill 尚不存在（upgrade-safety 外审 F7 订正：此前本行错把阻断原因记为 #28 墙）；`--wait` 同上（成功臂需真 re-exec + 新版本 re-register），hermetic 面由 `test/p10/upgrade_e2e_test.go` 与 `cmd/tether/node_upgrade_wait_test.go` 覆盖 | S5-31◑ | success + `--wait` deploy-tier 臂待 dedicated drill 立项（无 owner） |
...
- **S5**：`31-node-upgrade-fleet`（INCOMPLETE，**#28-FIXED** 前置 + fleet enumeration/dispatch/`--timeout` control oracle；真 re-exec / re-register / rollback 与 `--wait` deploy-tier success 仍 NOT-COVERED、尚无 owner）· `30-rolling-upgrade`（GREEN，13，N=3：**gotcha #31** grow lock 泄漏 + [GAP #31] + upgrade-roll NOT-COVERED）·
...
（--url/--sha256）→ **S5-31◑**（**#28 已修** + `--all` 枚举/dispatch/`--timeout` control 面 COVERED〔外审 M4 补臂〕；fleet SUCCESS 与 `--wait` 真 re-exec/re-register/rollback 仍 NOT-COVERED、尚无 owner）· `node ls --brokers` → S5-30✓（broker running version
