...
| S0-布局 | agent 二进制布局对齐 | install.sh 真实形态：agent 装用户可写 `~/.local/bin`（§1.4 债） | S5 | S5-31 | **已落地（G-A/S5；provision-node.sh ~/.local/bin 布局；`627e334`）** |
| S0-artifact | 发布制品库 | https 静态服务 + 实例作用域自签 CA 入容器 trust（OQ-3；CA 设施与 S0-ingress 共用） | S5 | S5-30/31 | **已落地（G-A/S3-S5；`drills/lib/artifact.sh`；`627e334`）** |
| S0-备份库 | 离簇备份库 | **实例命名空间化的 host 侧目录**：在 `rm_node --vols`（灾难注入）作用域**之外**、但在 `simcluster nuke` 作用域**之内**（nuke 学会删它——不跨轮泄漏状态）；fresh/空预检、0700、bundle 名唯一、trap 清理 | S7 | S7-50/51 | **已落地（G-C/S7；`lib/vault.sh` 离簇备份库，survives rm_node --vols，nuke 回收；`5f9ffec`）** |
| S0-故障原语 | 故障注入原语 | 目标 netns 内 nftables/iptables DROP / tc（分区）、SIGSTOP、卷级灾难（保留-secrets 变体）——每件带 Mandate ④ 说明模板 + 无条件清理 trap | S9（分区）/S7（卷级） | S9-96/97、S7-50/51 | **已落地（G-C/S7+S9；`drills/lib/fault.sh` iptables SIMFAULT 分区原语〔双 hook 双向〕+ leak.sh + events.sh；`5f9ffec`）** |
...
### 2.1 合并推进分组（merged execution groups）——S3–S9 以「组」为单位、一组一次外审

S1/S2 已单批落地（`10e9a5f` / `3bacc11`）。**其后剩余批 S3–S9 改按「组」推进**：一组内相邻批
合并成**单一 §3 流程**（一次 Stage-A 合并 plan → 一次连续实现 → 一次 Stage-C 内审 → **一次
外审**），外审次数 7→3。合并只改「一次流程覆盖几批」，**不改任何 drill 规格、依赖约束、§4 无
遗漏闸**——批仍是 §3/§4 的最小规格单位，组只是外审/落盘的粒度。分组四依据（优先级从高到低）：
① **内聚场景族**（一组=一族旅程，外审一次看得懂，§0.2）；② **共享 harness 增量就近**（同组一次
落地共用的 S0 项/lib，免跨组重复改同一文件）；③ **依赖顺序**（软依赖的被依赖批在同组前段或前
一组）；④ **控外审面**（粗粒度换少外审、细粒度换小审查面，拆点见下）。

**推荐三组（顺序即推进序 G-A → G-B → G-C）：**

| 组 | 含批 | 场景叙事 | drill（数） | 合并理由 | 本组落地 S0 |
|---|---|---|---|---|---|
| **G-A** | S3+S4+S5 | 服务对外暴露 + 保持最新（数据面 + 升级） | 70/71 · 72/73/74 · 30/31/32（8） | S3/S4=expose+proxy 同「对外暴露」族、共用**已落地**隧道/ingress；S5=版本运维、自带 artifact/布局 harness。清 **G7a/G5** 债 | S0-artifact、S0-布局（S5 自带） |
| **G-B** | S6+S8 | 调整拓扑 + 观察结果（收缩 + 客户端视图/告警） | 40/41/42/43/22 · 90/91/92/93（9） | S8-92(b) leg 软依赖 S6-22 结论 → 同组内 22 先跑、92 后用；收缩后看客户端视图/告警叙事连贯。清 **G3/G7b** 债 | 无新 S0 |
| **G-C** | S7+S9 | 灾难与混沌下的韧性（灾备 + 长稳收官） | 50/51/52 · 94/95/96/97（7） | 两批**共享 S0-故障原语 lib**（S7 卷级件 + S9 分区件）→ 同组一次建全部件；同为「注入故障→验恢复/对账」族。S9 soak 压轴 | S0-备份库、S0-故障原语（卷级+分区一次建） |

**固化的顺序约束**：G-A → G-B → G-C 不可乱序——G-B 必须在 G-C 前（S7 软依赖 S6 fixture 经验）；
G-B **组内** S6-22 必须在 S8-92 前（结论供 92(b) leg 消费；22 若判定 dwell-bounce 生产不可达成
则 92(b) 如实登记 NOT-COVERED，其余 leg 独立不受阻）。三组之间无其它硬依赖——S3–S9 需要的
隧道/ingress/pty/台账已由 S1/S2 全部落地（S0 表），未落地的 S0 项（artifact/布局/备份库/故障
原语）各自归属单一组、组间不争用。

**弹性拆点**（若单组外审面过大，任一组可退回细粒度、以多一次外审换小审查面，唯一归属规则与
S0 落地责任不变，S0 表）：G-A→{S3+S4}/{S5}；G-B→{S6}/{S8}；G-C→{S7}/{S9}。反向再合并亦可，但
**S7/S9 建议恒同组**（否则 S0-故障原语 lib 要分两次改同一文件）。组内实现顺序 = 批号顺序，
plan 在 Stage-A 一次覆盖全组、逐批分节（`s<组>-plan.md`，如 `s3-s5-plan.md`）。
