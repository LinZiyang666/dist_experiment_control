...
| `nilerr` ×5 | `internal/agent/upgrade.go` recoverFromFailedExec、`upgrade_state.go` watchdogRollback / commitUpgradeAfterRegister ×2、`conn_teardown.go` restorePrevForEscalation | 2026-08 upgrade-safety 外审修复轮追加（计数 30→34），follow-ups 外审 F4 再 +1（34→35） | flock 闭包的 error 返回只承载**锁**失败；marker 读损按契约是 idle、运行映像读不出按契约是「提交证明不成立」——返回 nil 是这五处的语义本体，不是吞错 |
