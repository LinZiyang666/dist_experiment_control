...
| c | 30 的 whole-host 断言依赖 roadmap **OQ-6** 的同机 agent 供给，而该供给从未实现 ⇒ 即便修好 H1 的 jq，在现有 sim 拓扑下仍不可满足（双重不可满足态）。按铁律②应二选一：真补 OQ-6 供给，或明确记 `not_covered` 并把断言改成只钉 broker 守护腿 | `drills/30-rolling-upgrade.sh:175`；`docs/reviews/simcluster-coverage-roadmap.md:448/:905-907` |
