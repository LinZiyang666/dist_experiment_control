...
file-transfer(push/pull,v0.2.0)、ps-retention(v0.2.8)、**P12 expose `--remote-port`**、**P13 proxy 订阅(§L)**、**transfer-unrestrict(v0.4.0)**、**remote-fs-resilience(v0.3.3)**、**cluster-phase-fluidity(v0.4.2,N=1↔2↔N grow/shrink)**。这些是叶子增量,沿用同一"plan → 实现 → 内审 → 外审"流程,但不阻塞主线里程碑。
...
> **cluster-phase-fluidity(v0.4.2,见 `docs/reviews/cluster-phase-fluidity-{plan,review,review-round2,external-review}.md`;外审 Fail→修 F1–F7→复审)**:让 N=1↔2↔N 的 grow/shrink 从**代码层面**顺畅流动,不再为一次常规 grow 动用 `force-single`(它退回为「只在真 quorum 丢失」的离线逃生)。
>
> **地址模型(external-review F7 校准)**:生产 `cluster.New` 永走 `existing` 分支。**peers 拨号的地址 = raft Configuration 的 server address**——由 `set-raft-addr` 经 `raft.AddVoter` 对**已存在 voter** 的**原地地址重写**(无 wipe/snapshot/replay,N=1 时 quorum=1 立即 commit)改,`cluster_nodes.raft_addr` 是其复制态镜像。**bind 与 advertise 分设**:transport 仍 bind 在启动期 `broker.cluster.raft_addr`(runbook §1.0 step a 先把它改 `0.0.0.0:7400` 再做一次干净重启,使 socket 在公网接口可达),advertise(Configuration 地址)再经 set-raft-addr **在线**改。pinned Hashicorp Raft 的 transport `LocalAddr()`(= bind 地址)确实进 raft RPC header,但 **tether 的 leader 观测只取 leader 的 node ID(`LeaderWithID` 的 addr 在所有调用点一律丢弃)**——leader 重定向走 node-id + NATS 转发(queue-group/broadcast,非 raft 地址直拨,见 `cluster_forward.go`),故 bind=`0.0.0.0` 对 tether 功能**惰性**:本叶子**有意不**实现 transport advertise/bind 解耦(`MTLSTransportConfig` 仅 `BindAddr`),因为 tether 不读 RPC-header 地址。
>
> **grow**:① 在线 `cluster set-raft-addr`(**自身-only**——改 peer 会 wedge,故 leader 只改自己;column-first Propose `OpClusterNodeReaddr` 后 `AddVoter`,写-free no-op + 碰撞拒);② **nonvoter 暂存(keystone)**——`driveJoin` 先 `AddNonvoter`(`sub.inRaft` 守卫)→ 追上 → `AddVoter` 提升,**加不可达 voter 不再 wedge N=1**(nonvoter 不计 quorum,旧 quorum 即 commit);失败/abort 的暂存 nonvoter **可在线清理**(external-review F1:`ops abort` 后 `recovery node remove` 接受 CATCHING_UP、防御性确认非已提交 voter、`PlanClusterNodeRemove` 走 mesh-leave 双 gen bump——不再回退 force-single);③ doctor loopback-advertise 提醒;`OpClusterNodeRoute` 改 nats_route(在签名 roster body + 渲染 conf,故 bump roster+topology gen)。**mixed-version 安全(external-review F5)**:`OpClusterNodeReaddr/Route` 是复制 op,旧 binary 的 `knownOps` 缺它会 poison-skip 致**确定性副本分叉**,故 set-raft-addr/set-route 前过 **capability gate**——每个非自身 voter 必须在 live health 报 `PhaseFluidityOps`,不可达/旧版 fail-closed(N=1 恒放行)。
>
> **shrink**:`reconcile nats --to-standalone --confirm-single`——`natscluster.Config.Standalone` 渲染**无 `cluster{}` 块**的 standalone conf(真 nats-server parser 验证)+ operator-gated clustered→standalone JS reset + 全量重启(脱簇非 SIGHUP-reloadable),**仅 N=1**。**fail-CLOSED(external-review F2)**:`--confirm-single` 只是 typed intent,实际门要 live status **恰好 1 voter** 证明,socket 不可达/未知 role/voters≠1 全拒(不再 fail-open)。**JS 守卫(external-review F3)**:source 开 JetStream 但无显式 `store_dir` 则拒(Render 在空 store_dir 下会静默删 jetstream{}),apply 后重 Preflight 断言 `IsStandaloneJetStream` + store_dir 不变。**持久 desired state(external-review F4 + R3)**:automatic topology reconciler **保当前 mode**——仅当 live conf **已是** standalone(`len(Peers)==1 && own.IsStandaloneJetStream()`)才渲染 standalone,故 N=1 standalone 是 reconciler 的真收敛态(survive restart/gen-bump/regrow、非一次性 CLI 改写);**仍 clustered 的 N=1 保持 clustered**,等待显式 `reconcile nats --to-standalone`——reconciler 绝不自动跨破坏性脱簇边界(R3:无 operator-intent 输入时不得静默删 `cluster{}`)。
>
...
>
> **本叶子有意 DEFER(external-review F7 重定,非 ship-blocker、登记为 open plan item)**:`cluster init --from-existing` 的 loopback-advertise 前置拒(在线 set-raft-addr 已覆盖、init 期 loopback 可在线恢复)、offline doctor 的 bind-vs-advertise 拆分、`join approve --check` 预检、transport advertise/bind 解耦(上述地址模型已论证 tether 不需要)、把 shrink 建成**复制态 operation**(当前是 CLI 一次性 + reconciler 持久 desired state 的组合)。多节点全集群实跑演练随发版前的实机验证补(项目既有 gated 集成 + 实机模式)。
