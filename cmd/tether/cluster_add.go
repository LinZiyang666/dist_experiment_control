...
func newClusterAddCmd(socketPath *string) *cobra.Command {
	...
	cmd.Flags().BoolVar(&resetFormerJS, "reset-former-js", false, "acknowledge resetting a NON-EMPTY JetStream store on EITHER end of the grow — the former-N1 survivor's clustered store AND a RETURNING joiner's stale store (history/audit loss; the store is MOVED aside, not deleted)")
	cmd.Flags().BoolVar(&preserveJSData, "preserve-js-data", false, "acknowledge the JS reset (either end of the grow), keeping the moved-aside store (jetstream.grow-bak.<epoch>) to restore by hand; auto backup→restore is not implemented in v1")
