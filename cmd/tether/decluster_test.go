...
// origin: batch B2 external re-review of the rejected half of B2-1.
//
// The topology still has to lose cluster{} when JetStream is explicitly disabled, but disabled
// JetStream does not require a store_dir. Treating raw key presence as both facts enters the de-cluster
// branch and then refuses it, contradicting the reason given for retaining the presence predicate.
func TestDeClusterExplicitlyDisabledJetStreamDoesNotRequireAStoreDir(t *testing.T) {
	dir := t.TempDir()
	confPath := filepath.Join(dir, "nats.conf")
	const enabled = `jetstream {
  store_dir: "/var/lib/tether/js"
}`
	disabled := strings.Replace(g2ClusteredMultiUserConf, enabled, "jetstream: false", 1)
	if disabled == g2ClusteredMultiUserConf {
		t.Fatal("fixture replacement did not match")
	}
	if err := os.WriteFile(confPath, []byte(disabled), 0o600); err != nil {
		t.Fatal(err)
	}
	dbPath := filepath.Join(dir, "tether.db")
	seedSelfIdentity(t, dbPath, "brk-a", "brk-a", "USELFBROKERNKEY")

	merged, _, already, err := buildStandaloneConf(confPath, "brk-a", dbPath)
	if err != nil {
		t.Fatalf("explicitly disabled JetStream still cannot be de-clustered: %v. The cluster{} block is "+
			"NATS topology and must be removed, but a disabled subsystem must not be forced to invent a "+
			"JetStream store_dir.", err)
	}
	if already {
		t.Fatal("cluster{} is still present, so this is not an already-standalone no-op")
	}
	if strings.Contains(merged, "cluster {") || strings.Contains(merged, "routes:") {
		t.Fatalf("de-cluster render retained clustered NATS topology:\n%s", merged)
	}
}
