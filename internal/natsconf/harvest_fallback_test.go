...
func TestBuildMergedConfStandaloneHasNothingToHarvest(t *testing.T) {
	...
	if !own.IsStandaloneTopology() {
		t.Fatal("fixture must have NO cluster{} block — that absence is the whole premise of the harvest " +
			"fallback under test (RB2-1: this is a topology question, not a JetStream one)")
	}
