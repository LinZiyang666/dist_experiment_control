...
// TestPerformGrowCutoverRefusesDisabledJetStreamBeforeClusteredFastPaths keeps the topology/enablement
// split honest at the Stage-A/Stage-B resume paths. A clustered block is evidence of topology only; it
// must not bypass the requirement that this operation is growing a JetStream mesh.
//
// origin: batch B2 independent external second re-review
func TestPerformGrowCutoverRefusesDisabledJetStreamBeforeClusteredFastPaths(t *testing.T) {
	b := newCutoverTestBroker(t)
	b.cfg.Logger = silentLogger()
	b.cfg.NatsServerBin = "/bin/false"
	n, _ := d7SingleNode(t, "brk-a")
	b.cl = &clusterRuntime{node: n}

	disabledClustered := strings.Replace(g2ClusteredConf,
		"jetstream {\n  store_dir: \"/var/lib/tether/js\"\n}", "jetstream: false", 1)
	b.cfg.NatsConfPath = writeConfFile(t, disabledClustered)
	const epoch = "disabled-js-epoch"
	if err := os.WriteFile(filepath.Join(b.cfg.ClusterDataDir, ".grow-cutover-"+epoch+".done"),
		[]byte("fixture"), 0o600); err != nil {
		t.Fatal(err)
	}

	// Make the topology probe deterministic and quick. It reports an up, standalone process, so reaching
	// Stage B invokes /bin/false as the restart command and exposes that the JS guard was placed too late.
	oldProbeClient := topoProbeClient
	topoProbeClient = &http.Client{Transport: externalReviewRoundTripper(func(*http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Body:       io.NopCloser(strings.NewReader(`{"cluster":{"name":""}}`)),
			Header:     make(http.Header),
		}, nil
	})}
	t.Cleanup(func() { topoProbeClient = oldProbeClient })

	resp := b.performGrowCutover(&proto.ClusterGrowReq{Op: "mesh-cutover", TargetNode: "brk-a", GrowEpoch: epoch})
	if resp == nil || resp.Code != adminsock.CodeBadRequest ||
		!strings.Contains(strings.ToLower(resp.Error), "jetstream disabled") {
		t.Fatalf("clustered topology with JetStream explicitly disabled must fail at the enablement guard "+
			"before AlreadyDone/restart-only fast paths; got %+v", resp)
	}
}
