...
// fakeNatsServer writes an executable that ignores its args and exits 0, so natsconf.DryRun's
...
func g4GrowPeers() []natscluster.Broker {
...
func TestReconcileWithholdsClusteredCutoverSwap(t *testing.T) {
	...
	if err := os.WriteFile(conf, []byte(g4StandaloneConf), 0o600); err != nil {
		t.Fatal(err)
	}
	in := Inputs{
	...
	if string(after) != g4StandaloneConf {
